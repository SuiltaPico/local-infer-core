# Build infer-core-ffi for Android (arm64-v8a + x86_64) with MNN backend.
param(
...
function Resolve-NdkLibcxxShared {
    ...
    $triple = switch ($Abi) {
        ...
        default { throw "unsupported Android ABI for libc++_shared: $Abi" }
    }
...
try {
    ...
    foreach ($name in $Abis) {
        ...
        Write-Host "MNN_LINK: $($env:MNN_LINK)"
        Write-Host "Building Android ABI: $name (backend-mnn, no ORT)"
        ...
        Write-Host "Built: android/jniLibs/$name/libinfer_core.so ($sizeMb MB, $libCount .so files)"
    }
} finally {
    Pop-Location
}
