//! SVG → PNG rasterization for icon index build (MDI / Tabler / Fluent / FA templates).
...
impl IconRasterColor {
    pub fn parse(s: &str) -> Result<Self> {
        match s.to_ascii_lowercase().as_str() {
            "black" => Ok(Self::Black),
            "white" => Ok(Self::White),
            other => Err(anyhow!("invalid color {other:?} (expected black or white)")),
        }
    }
}
