...
impl Interpreter {
    ...
    /// Forward types selected for this session (`getSessionInfo` / `BACKENDS`).
    pub fn backends(&self, session: &crate::Session) -> Result<Vec<i32>> {
        let mut backends = [0i32; 8];
        let ret = unsafe {
            mnn_sys::Interpreter_getSessionInfo(
                self.inner,
                session.inner,
                mnn_sys::cpp::MNN_Interpreter_SessionInfoCode_BACKENDS as _,
                backends.as_mut_ptr().cast(),
            )
        };
        ensure!(
            ret == 1,
            ErrorKind::InterpreterError;
            "Failed to get session backends"
        );
        Ok(backends
            .into_iter()
            .filter(|code| *code != 0)
